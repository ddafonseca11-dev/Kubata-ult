...
import { getLeadsPaginated } from '@/services/adminService';
...
export default function AdminLeads() {
  ...
  } = useAdminPagination<Lead>({ fetcher: getLeadsPaginated });

  return (
    <div>
      <div className="flex flex-wrap gap-3 mb-4">
        <AdminSearchBar value={search} onChange={handleSearch} placeholder="Nome, email, telefone..." />
        ...
        <select value={filters.source || ''} onChange={(e) => handleFilterChange('source', e.target.value)} className="px-3 py-2 rounded-lg border border-slate-300 text-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                <th className="text-left px-4 py-3 font-medium text-slate-600 cursor-pointer hover:text-slate-900" onClick={() => handleSort('name')}>Nome {sortBy === 'name' && (sortOrder === 'asc' ? '↑' : '↓')}</th>
                ...
                <th className="text-right px-4 py-3 font-medium text-slate-600">Orçamento</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td colSpan={6} className="text-center py-8 text-slate-400">Nenhum lead encontrado.</td></tr>
              ) : (
                data.map((lead) => (
                  <tr key={lead.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                        ...
                        lead.status === 'qualified' ? 'bg-teal-100 text-teal-700' :
                        lead.status === 'converted' ? 'bg-green-100 text-green-700' :
                        lead.status === 'lost' ? 'bg-red-100 text-red-700' :
                        'bg-slate-100 text-slate-600'
                      }`}>{lead.status}</span>
                    </td>
                    <td className="px-4 py-3 text-right text-slate-900">{lead.budget ? `${lead.budget}€` : '—'}</td>
                    <td className="px-4 py-3 text-slate-500">{new Date(lead.created_at).toLocaleDateString('pt-PT')}</td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      <div className="text-sm text-slate-500 mt-2">{count} leads</div>
